# reinforcement_learning
This directory contains projects for building Machine Learning reinforcement learning models.

## q_learning
This directory contains initial work with reinforcement learning through Q-learning:

0. [Load the Environment](/reinforcement_learning/q_learning/0-load_env.py)
1. [Initialize Q-table](/reinforcement_learning/q_learning/1-q_init.py)
2. [Epsilon Greedy](/reinforcement_learning/q_learning/2-epsilon_greedy.py)
3. [Q-learning](/reinforcement_learning/q_learning/3-q_learning.py)
4. [Play](/reinforcement_learning/q_learning/4-play.py)

